...
static int video_open(VideoState *is, int force_set_video_mode)
{
	...
	//FIX
	GetClientRect(g_hWnd,&hwnd_rc);
	screen = SDL_SetVideoMode(hwnd_rc.right-hwnd_rc.left, hwnd_rc.bottom-hwnd_rc.top, 0, flags);
...
#define __MINGW32__
int ff_start(char *url,HWND hWnd)
{
    int flags;
    VideoState *is;
...
//    parse_loglevel(argc, argv, options);
   ...
   // init_opts();
	...
	if(hWnd!=NULL){
		g_hWnd=hWnd;
		char sdl_var[128]; 
		sprintf(sdl_var, "SDL_WINDOWID=0x%lx", hWnd );//Ö÷´°¿Ú¾ä±ú 
		SDL_putenv(sdl_var); 
